pipeline {
    ...
    stages {
        stage('Docker') {
            ...
            when {
                anyOf {
                    branch pattern: 'sif-.*', comparator: "REGEXP";
                    buildingTag();
                }
            }
            environment {
            ...
            stages {
                stage('Prepare') {
                    steps {
                        script {
                            if (env.BRANCH_NAME.startsWith('sif-')) {
                            ...
                            env.SOURCE_BASE_URL = "https://github.com/CyberSiliconForest/mastodon" // I'm lazy. Will fix it later
